...
layout = [
    ...
    [sg.Exit('关闭窗口', font=['黑体', '10'], bind_return_key=True, button_color=('white', 'green')),
     ...
     sg.Button('开机启动设置', font=['黑体', '10'], bind_return_key=True, button_color=('white', 'green'))]]
        ...
        with open('jwxtinfo', 'w') as f:
            layout_input = [[sg.Text('请输入教务系统账号和密码！')],
                            ...
                            [sg.Button('验证密钥', bind_return_key=True),
                             ...
                             sg.Checkbox('开机启动', size=(12, 1), key='_BOX_', default=True)]]
            window_input = sg.Window('BjfuJwxt', layout_input, keep_on_top=True)
            event, result = window_input.Read()
            Usn, Psw, Key, Box = result['_UN_'], result['_PW_'], result['_KEY_'], result['_BOX_']
            ...
            if Box is True:
                path = os.getcwd()
                split = path.split('\\')
                if split[0] + split[1] + split[-1] == 'C:UsersDesktop':
                    split = split[:-1]
                    split.extend(['AppData', 'Roaming', 'Microsoft', 'Windows', 'Start Menu', 'Programs', 'Startup'])
                    newpath = '\\'.join(split)
                    with open('jwxtinfo', 'a') as fn:
                        fn.write(newpath)
                        fn.close()
                    shutil.copy(path + '\\BjfuScoreGUI.exe', newpath)
                    shutil.copy(path + '\\jwxtinfo', newpath)
                    win32api.SetFileAttributes(path + '\\jwxtinfo', win32con.FILE_ATTRIBUTE_HIDDEN)
                else:
                    sg.PopupOK('请将文件放在桌面再运行')
            window_input.Close()
...
if __name__ == '__main__':
    ...
    for i in range(3):
        try:
            ...
            time.sleep(20)
            if i == 2:
                sg.PopupOK('成绩监测小程序\n---登录失败---\n--请检查网络--', no_titlebar=True, text_color='red',
                           font=['黑体', '10'], keep_on_top=True, button_color=('white', 'red'))
                sys.exit()
    new.login()  # 登录
    new.info()  # 账户信息
    ...
    if event == '开机启动设置':
        with open('jwxtinfo', 'r') as f:
            ...
            f.close()
            if len(lines) == 4:
                layout_start = [[sg.Text('请选择是否开机启动')],
                                [sg.Checkbox('开机启动', size=(12, 1), key='_BOX_', default=True)],
                                [sg.Button('OK', bind_return_key=True)]]
                window_input = sg.Window('Start', layout_start, keep_on_top=True)
                event, result = window_input.Read()
                Box = result['_BOX_']
                if Box is True:
                    pass
                else:
                    os.remove(lines[-1] + '\\BjfuScoreGUI.exe')
                    os.remove(lines[-1] + '\\jwxtinfo')
                    os.remove('jwxtinfo')
                    with open('jwxtinfo', 'w') as fw:
                        fw.write(lines[0] + lines[1] + lines[2])
                        fw.close()
                    win32api.SetFileAttributes('jwxtinfo', win32con.FILE_ATTRIBUTE_HIDDEN)
            if len(lines) == 3:
                layout_start = [[sg.Text('请选择是否开机启动')],
                                [sg.Checkbox('开机启动', size=(12, 1), key='_BOX_', default=False)],
                                [sg.Button('OK', bind_return_key=True)]]
                window_input = sg.Window('Start', layout_start, keep_on_top=True)
                event, result = window_input.Read()
                Box = result['_BOX_']
