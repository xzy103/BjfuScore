...
layout = [
    ...
    [sg.Exit('关闭窗口', font=['黑体', '10'], bind_return_key=True, button_color=('white', 'green')),
     ...
     sg.Button('开机启动目录', font=['黑体', '10'], bind_return_key=True, button_color=('white', 'green'), key='_START_')]]
window = sg.Window('BjfuJwxt', layout, background_color='white', no_titlebar=True, resizable=True, grab_anywhere=True)
...
class Newjwxt(object):
    ...
    def saveclasses(self):
        ...
        if content == score:
            ...
            guiprint('>>>>>成绩信息已更新')
            sg.PopupOK('成绩有更新！', no_titlebar=True, text_color='green', font=['黑体', '12'],
                       button_color=('white', 'green'), keep_on_top=True)
        self.write()
    ...
    def refeshclasses(self):
        ...
        if content == score:
            ...
            sg.PopupOK('成绩有更新！\n' + str(table_new), auto_close=True, keep_on_top=True,
        ...
        with open('jwxtinfo', 'w') as f:
            layout_input = [[sg.Text('请输入教务系统账号和密码！')],
                            ...
                            [sg.Button('验证密钥', bind_return_key=True),
                             sg.Button('退出', bind_return_key=True)]]
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            time.sleep(20)
            if flag:
                sg.PopupOK('请检查网络！', no_titlebar=True, text_color='red', keep_on_top=True,
                           font=['黑体', '10'], button_color=('white', 'red'))
                flag = 0
    ...
    if event == '_START_':
        sg.PopupOK('将程序复制到\n[开机启动目录]\n可实现开机启动\n但此法尚不稳定\n如遇安全软件请放行',
                   keep_on_top=True, no_titlebar=True, font=['黑体', '10'])
        pc_user = getpass.getuser()
        newpath = 'C:\\Users\\' + pc_user + '\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Startup'
        os.startfile(newpath)
        window.Element('_START_').Update(visible=False)
    ...
    if event == '后台监测':
        sg.PopupNoButtons('20min刷新一次\n持续监测中...\n程序将转入后台运行', auto_close_duration=3, no_titlebar=True,
                          keep_on_top=True, auto_close=True, text_color='green', font=['黑体', '10'])
        while True:
            try:
                check()
                new = Newjwxt(UserName, PassWord)
                ...
                sg.PopupOK('登录超时或网络断开\n连接网络后程序会继续运行', no_titlebar=True, text_color='green',
                           font=['黑体', '10'], button_color=('white', 'green'), keep_on_top=True)
                while True:
                    try:
                        check()
                        sg.PopupOK('网络已恢复', no_titlebar=True, text_color='green', keep_on_top=True,
                                   font=['黑体', '10'], button_color=('white', 'green'))
                        break
                    except:
                        time.sleep(60)
